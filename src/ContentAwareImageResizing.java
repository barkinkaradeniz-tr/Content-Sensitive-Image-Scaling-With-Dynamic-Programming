...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        // TODO
        int[] VPath = new int[sy];

        WeightedDigraph wd = makeVGraph();
        ShortestPathsTopological spt = new ShortestPathsTopological(wd, sx*sy);

        Stack<Integer> order = spt.pathTo(sx * sy + 1);

        for (int i = 0; i < spt.pathTo(sx * sy + 1).size()-2; i++) {
            if ((i == 0)) {
                order.pop();
            }

            VPath[i] = order.pop();
            VPath[i] = nodeToCoordinate(VPath[i]).x;
        }
